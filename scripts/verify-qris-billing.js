...
class QRISBillingVerification {
  ...
  async testInvoiceCreation() {
    ...
    try {
      ...
      this.addResult(
        ...
        subscriptionInvoice &&
          subscriptionInvoice.publicId &&
          subscriptionInvoice.qrisPayload,
      ...
      this.addResult(
        ...
        deliveryInvoice &&
          deliveryInvoice.publicId &&
          deliveryInvoice.qrisPayload,
  ...
  async testQRISGeneration() {
    ...
    try {
      ...
      const isValidFormat =
        qrisPayload.startsWith("00020101021226") &&
    ...
    try {
      ...
      try {
        ...
        this.addResult(
          ...
          false,
          "Should have thrown error for duplicate payment"
        );
      } catch (error) {
        this.addResult(
          "Payment Confirmation - Duplicate Prevention",
      ...
      this.addResult(
        ...
        deliveryPaymentResult &&
          deliveryPaymentResult.invoice.status === "paid",
  ...
  async testDriverLimitEnforcement() {
    ...
    try {
      ...
      const currentDrivers = await this.driverService.listDriversForMitra(
        this.testMitra1.id
      );
      const driversToInvite =
        TEST_CONFIG.DEFAULT_DRIVER_LIMIT - currentDrivers.length + 1; // Exceed limit by 1
      ...
      const paymentRequiredErrors = inviteResults.filter(
        r => !r.success && r.code === "PAYMENT_REQUIRED"
      ).length;

      this.addResult(
        "Driver Limit Enforcement - Free Tier",
        successfulInvites <= TEST_CONFIG.DEFAULT_DRIVER_LIMIT &&
          paymentRequiredErrors > 0,
      ...
      const activeCurrentDrivers = await this.driverService.listDriversForMitra(
        this.testMitra2.id
      );
      const canInviteMore =
        TEST_CONFIG.PREMIUM_DRIVER_LIMIT - activeCurrentDrivers.length;
  ...
  async testIdempotentMonthlyInvoiceGeneration() {
    ...
    try {
      ...
      const activeInvoices = await this.billingService.getInvoicesByMitra(
        this.testMitra2.id,
        "pending"
      );
      const monthlySubscriptionInvoices = activeInvoices.filter(
        inv =>
      ...
      if (monthlySubscriptionInvoices.length > 0) {
        ...
        const expectedAmount =
          this.testMitra2.activeDriverLimit *
          TEST_CONFIG.SUBSCRIPTION_AMOUNT_PER_DRIVER;
  ...
  async testInvoiceRetrieval() {
    ...
    try {
      ...
      const mitra1Invoices = await this.billingService.getInvoicesByMitra(
        this.testMitra1.id
      );
      const mitra2Invoices = await this.billingService.getInvoicesByMitra(
        this.testMitra2.id
      );
      ...
      const pendingInvoices = await this.billingService.getInvoicesByMitra(
        this.testMitra1.id,
        "pending"
      );
      const paidInvoices = await this.billingService.getInvoicesByMitra(
        this.testMitra1.id,
        "paid"
      );
    ...
    try {
      ...
      try {
        ...
        this.addResult(
          ...
          false,
          "Should have thrown foreign key constraint error"
        );
      } catch (error) {
        this.addResult(
          "Database Constraints - Foreign Key",
  ...
  generateReport() {
    ...
    if (criticalPassed === criticalTotal) {
      ...
      this.log(
        "The Manual QRIS Billing System implementation is ready for production."
      );
    } else {
      this.log("\n⚠️  CRITICAL ISSUES DETECTED");
      this.log(
        "Please review and fix the failing tests before deploying to production."
      );
    }
