...
// Generate JWTs
const tokens = {
  mitra1: jwt.sign({ userId: users.mitra1.userId }, JWT_SECRET, {
    expiresIn: "1h",
  }),
  mitra2: jwt.sign({ userId: users.mitra2.userId }, JWT_SECRET, {
    expiresIn: "1h",
  }),
  driver1: jwt.sign({ userId: users.driver1.userId }, JWT_SECRET, {
    expiresIn: "1h",
  }),
};
...
async function test1_1_successfulInvitation() {
  ...
  if (result.status === 200 && result.data.inviteLink) {
    ...
    console.log(
      `✅ PASS: Response contains inviteLink: ${result.data.inviteLink}`
    );
    ...
    console.log(
      `❌ FAIL: Expected 200 with inviteLink, got ${result.status}`,
      result.data
    );
    return false;
  }
...
async function test1_2_invitationFailures() {
  ...
  if (nonMitraResult.status === 403) {
    ...
    console.log(
      `❌ FAIL: Expected 403 for non-Mitra JWT, got ${nonMitraResult.status}`
    );
    allPassed = false;
  }
  ...
  if (invalidEmailResult.status === 400) {
    ...
    console.log(
      `❌ FAIL: Expected 400 for invalid email, got ${invalidEmailResult.status}`
    );
    allPassed = false;
  }
  ...
  if (duplicateResult.status === 409) {
    ...
    console.log(
      `❌ FAIL: Expected 409 for duplicate invitation, got ${duplicateResult.status}`
    );
    allPassed = false;
  }
...
async function test2_1_successfulAcceptance() {
  ...
  if (
    ...
    console.log(
      `❌ FAIL: Expected 200 with success message, got ${result.status}`,
      result.data
    );
    return false;
  }
...
async function test2_2_acceptanceFailures() {
  ...
  if (invalidTokenResult.status === 400) {
    ...
    console.log(
      `❌ FAIL: Expected 400 for invalid token, got ${invalidTokenResult.status}`
    );
    allPassed = false;
  }
  ...
  if (duplicateAcceptResult.status === 409) {
    ...
    console.log(
      `❌ FAIL: Expected 409 for duplicate acceptance, got ${duplicateAcceptResult.status}`
    );
    allPassed = false;
  }
...
async function test3_1_listDrivers() {
  ...
  if (
    ...
    if (
      driver.id &&
      driver.userId === users.driver1.userId &&
    ...
    console.log(
      `❌ FAIL: Expected 200 with array of 1 driver, got ${result.status}`,
      result.data
    );
    return false;
  }
...
async function test3_2_removeDriver() {
  ...
  if (result.status === 204) {
    ...
    if (
      ...
      console.log(
        "❌ FAIL: Drivers list should be empty after removal",
        listResult.data
      );
      return false;
    }
    ...
    console.log(
      `❌ FAIL: Expected 204 for driver removal, got ${result.status}`
    );
    return false;
  }
}
...
async function test4_multiTenancySecurity() {
  ...
  if (
    mitra2DriversResult.status === 200 &&
    Array.isArray(mitra2DriversResult.data) &&
    mitra2DriversResult.data.length === 0
  ) {
    console.log(
      "✅ PASS: Mitra 2 sees empty drivers list (proper tenant isolation)"
    );
  } else {
    console.log(
      `❌ FAIL: Mitra 2 should see empty list, got ${mitra2DriversResult.status}`,
      mitra2DriversResult.data
    );
    allPassed = false;
  }
