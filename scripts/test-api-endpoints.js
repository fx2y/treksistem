...
try {
  ...
  const checks = [
    {
      pattern: /notifications\.post\(.*triggered.*async/,
      desc: "POST /:logId/triggered endpoint",
    },
    { pattern: /c\.req\.param\(\)/, desc: "Route parameter extraction" },
    {
      pattern: /\.update\(notificationLogs\)/,
      desc: "Database update operation",
    },
    {
      pattern: /\.set\(\{ status: "triggered" \}\)/,
      desc: "Status update to 'triggered'",
    },
  ...
  console.log(
    `📝 Notifications API: ${passed}/${checks.length} checks passed\n`
  );
} catch (err) {
...
try {
  ...
  const adminChecks = [
    ...
    {
      pattern: /admin\.get.*\/notifications\/templates/,
      desc: "GET templates endpoint",
    },
    {
      pattern: /admin\.post.*\/notifications\/templates/,
      desc: "POST templates endpoint",
    },
    {
      pattern: /admin\.get.*\/notifications\/templates\/:id/,
      desc: "GET template by ID endpoint",
    },
    {
      pattern: /admin\.put.*\/notifications\/templates\/:id/,
      desc: "PUT template endpoint",
    },
    {
      pattern: /admin\.delete.*\/notifications\/templates\/:id/,
      desc: "DELETE template endpoint",
    },
    {
      pattern: /admin\.post.*\/templates\/seed/,
      desc: "POST seed templates endpoint",
    },
  ...
  console.log(
    `🔐 Admin API: ${adminPassed}/${adminChecks.length} checks passed\n`
  );
} catch (err) {
...
try {
  ...
  const serviceChecks = [
    ...
    {
      pattern: /recipientPhone.*formattedPhone/,
      desc: "Phone number formatting",
    },
    {
      pattern: /generateNotification.*function/,
      desc: "Legacy compatibility function",
    },
  ];
  ...
  console.log(
    `⚙️  NotificationService: ${servicePassed}/${serviceChecks.length} checks passed\n`
  );
} catch (err) {
...
try {
  ...
  const schemaChecks = [
    {
      pattern: /export const notificationTemplates/,
      desc: "notificationTemplates table",
    },
    {
      pattern: /export const notificationLogs/,
      desc: "notificationLogs table",
    },
    { pattern: /type.*text.*notNull/, desc: "Template type field" },
    {
      pattern: /language.*text.*default.*id/,
      desc: "Language field with default",
    },
    { pattern: /content.*text.*notNull/, desc: "Template content field" },
    {
      pattern: /status.*enum.*generated.*triggered.*failed/,
      desc: "Log status enum",
    },
    { pattern: /orderId.*references.*orders\.id/, desc: "Order foreign key" },
    {
      pattern: /templateId.*references.*notificationTemplates\.id/,
      desc: "Template foreign key",
    },
  ...
  console.log(
    `🗄️ Database Schema: ${schemaPassed}/${schemaChecks.length} checks passed\n`
  );
} catch (err) {
...
try {
  ...
  depChecks.forEach(check => {
    const found =
      packageData.dependencies && packageData.dependencies[check.key];
    console.log(`  ${found ? "✅" : "❌"} ${check.desc} (${check.key})`);
    if (found) depPassed++;
  });

  console.log(
    `📦 Dependencies: ${depPassed}/${depChecks.length} checks passed\n`
  );
} catch (err) {
