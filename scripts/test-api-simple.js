...
async function testAPI() {
  ...
  try {
    ...
    if (inviteResponse.ok) {
      ...
      if (inviteToken) {
        console.log(`✅ PASS: Token extracted: ${inviteToken.substring(0, 10)}...`);
        
        // Test 2: Accept invitation
        console.log('\n2. Testing invitation acceptance...');
        const acceptResponse = await fetch(`${API_BASE}/api/public/invites/accept`, {
          method: 'POST',
          headers: {
        ...
        if (acceptResponse.ok) {
          ...
          if (listResponse.ok) {
            ...
            if (listData.length > 0 && listData[0].id) {
              ...
              const removeResponse = await fetch(`${API_BASE}/api/mitra/drivers/${driverId}`, {
                method: 'DELETE',
                headers: {
                  'Authorization': `Bearer ${mitra1Token}`
                }
              ...
              if (removeResponse.status === 204) {
                ...
                const emptyListResponse = await fetch(`${API_BASE}/api/mitra/drivers`, {
                  method: 'GET',
                  headers: {
                    'Authorization': `Bearer ${mitra1Token}`
                  }
                ...
                if (emptyListResponse.ok) {
                  ...
                  if (Array.isArray(emptyListData) && emptyListData.length === 0) {
                ...
                console.log(`❌ FAIL: Driver removal failed with status ${removeResponse.status}`);
              }
            }
          } else {
            console.log(`❌ FAIL: Driver listing failed with status ${listResponse.status}`);
          }
        } else {
          console.log(`❌ FAIL: Invitation acceptance failed with status ${acceptResponse.status}`);
          const errorText = await acceptResponse.text();
          console.log('Error:', errorText);
        }
      }
    } else {
      console.log(`❌ FAIL: Invitation creation failed with status ${inviteResponse.status}`);
