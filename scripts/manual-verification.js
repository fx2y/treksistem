...
const adminEndpoints = [
  '/api/admin/mitras',
  '/api/admin/test-data/setup'
];

for (const endpoint of adminEndpoints) {
  const result = await apiCall(endpoint, {
    headers: { 'Authorization': `Bearer ${jwt.sign({ userId: 'admin' }, JWT_SECRET)}` }
  });
  console.log(`${endpoint}: ${result.status} - ${JSON.stringify(result.data).slice(0, 100)}...`);
}
}
