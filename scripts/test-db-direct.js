...
async function runQuery(sql) {
  try {
    const { stdout } = await execAsync(`wrangler d1 execute treksistem-db-local --local --command="${sql}"`);
    const jsonMatch = stdout.match(/\[\s*{\s*"results"[\s\S]*?\s*\]\s*(?=\[33m|\s*$)/);
    if (jsonMatch) {
...
async function testDatabase() {
  ...
  if (mitra02) {
    console.log(`✅ Mitra02 (${mitra02.business_name}) - Status: ${mitra02.subscription_status}`);
  }
  
  const custInvoice = invoices.find(i => i.public_id === 'pub_cust_inv01');
  if (custInvoice) {
    console.log(`✅ Customer invoice pub_cust_inv01 - Amount: ${custInvoice.amount}, Type: ${custInvoice.type}`);
  }
  
  const mitra01Limits = driverLimits.find(m => m.id === 'mitra01');
  if (mitra01Limits) {
    console.log(`✅ Mitra01 driver limits - Current: ${mitra01Limits.current_drivers}/${mitra01Limits.active_driver_limit}`);
  }
