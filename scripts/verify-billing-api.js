...
class BillingAPIVerification {
  ...
  async testMitraInvoicesAPI() {
    ...
    try {
      ...
      const pendingResponse = await this.makeRequest(
        "/api/mitra/billing/invoices?status=pending",
        {
          token: this.tokens.mitra2,
        }
      );
      ...
      this.addResult(
        ...
        emptyResponse.status === 200 &&
          emptyResponse.data.invoices?.length === 0,
        `Invoices: ${emptyResponse.data.invoices?.length || 0}`
      );

      // Test: GET /api/mitra/billing/invoices/:invoiceId (mitra02)
      const singleInvoiceResponse = await this.makeRequest(
        "/api/mitra/billing/invoices/pub_inv01",
        {
          token: this.tokens.mitra2,
        }
      );
      ...
      this.addResult(
        ...
        singleInvoiceResponse.status === 200 &&
          singleInvoiceResponse.data.status === "pending",
      ...
      const createInvoiceResponse = await this.makeRequest(
        "/api/mitra/invoices",
        {
          method: "POST",
          token: this.tokens.mitra2,
  ...
  async testAdminInvoicesAPI() {
    ...
    try {
      ...
      const confirmPaymentResponse = await this.makeRequest(
        "/api/admin/invoices/inv03/confirm-payment",
        {
          method: "POST",
      ...
      const unauthorizedConfirmResponse = await this.makeRequest(
        "/api/admin/invoices/inv03/confirm-payment",
        {
          method: "POST",
          token: this.tokens.mitra2,
  ...
  async testPublicPaymentAPI() {
    ...
    try {
      ...
      const publicPaymentResponse = await this.makeRequest(
        "/pay/pub_cust_inv01"
      );
      ...
      this.addResult(
        ...
        publicPaymentResponse.data.qrisPayload &&
          typeof publicPaymentResponse.data.qrisPayload === "string",
  ...
  async testSubscriptionEnforcement() {
    ...
    try {
      ...
      const pastDueInviteResponse = await this.makeRequest(
        "/api/mitra/drivers/invite",
        {
          method: "POST",
      ...
      const freeTierInviteResponse = await this.makeRequest(
        "/api/mitra/drivers/invite",
        {
          method: "POST",
          token: this.tokens.mitra1,
      ...
      this.addResult(
        ...
        freeTierInviteResponse.status === 402 ||
          freeTierInviteResponse.status === 201,
      ...
      const activeInviteResponse = await this.makeRequest(
        "/api/mitra/drivers/invite",
        {
          method: "POST",
          token: this.tokens.mitra2,
      ...
      this.addResult(
        ...
        activeInviteResponse.status === 201 ||
          activeInviteResponse.status === 402,
    ...
    try {
      ...
      this.addResult(
        ...
        healthResponse.status === 200 &&
          healthResponse.data.includes("Treksistem"),
