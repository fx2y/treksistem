...
*   **Accessing Context in a Route Handler:**
    ...
    app.post('/services', async (c) => {
      ...
      const newService = await mitraServiceManagement.createService(mitraId, body);
...
*   **File Uploads (2-Step Process):**
    ...
    const { signedUrl, publicUrl } = await apiClient.post('/api/uploads/request-url', { fileName, contentType });

    // 2. Frontend: Upload the file directly to R2 using the signed URL
    await fetch(signedUrl, { method: 'PUT', body: file });

    // 3. Frontend: Save the public URL with the report
    await apiClient.post('/api/driver/orders/.../report', { photoUrl: publicUrl });
    ```
