import { createDbClient, users, mitras, vehicles, drivers, services, orders, orderStops, orderReports } from "@treksistem/db";
...
app.post("/setup-logbook", async c => {
  ...
  try {
    ...
    await db.exec(`DELETE FROM order_reports WHERE id IN ('report_1_pickup', 'report_1_dropoff', 'report_2_pickup', 'report_2_dropoff')`);
    await db.exec(`DELETE FROM order_stops WHERE id IN ('stop_1_pickup', 'stop_1_dropoff', 'stop_2_pickup', 'stop_2_dropoff')`);
    await db.exec(`DELETE FROM orders WHERE id IN ('order_1', 'order_2')`);
    await db.exec(`DELETE FROM services WHERE id = 'service_1'`);
    await db.exec(`DELETE FROM vehicles WHERE id IN ('vehicle_v1', 'vehicle_v2')`);
    await db.exec(`DELETE FROM drivers WHERE id = 'driver_a1'`);

    // Create driver for Mitra 1
    await db.exec(`INSERT INTO drivers (id, user_id, mitra_id, status) VALUES ('driver_a1', 'user_driver_1', 'mitra_1', 'active')`);

    // Create vehicles for Mitra 1
    await db.exec(`INSERT INTO vehicles (id, mitra_id, license_plate, description) VALUES ('vehicle_v1', 'mitra_1', 'N1234ABC', 'Toyota Avanza 2020')`);
    await db.exec(`INSERT INTO vehicles (id, mitra_id, license_plate, description) VALUES ('vehicle_v2', 'mitra_1', 'N5678XYZ', 'Honda Brio 2021')`);

    // Create service for Mitra 1
    await db.exec(`INSERT INTO services (id, mitra_id, service_name, description, is_active) VALUES ('service_1', 'mitra_1', 'Food Delivery Service', 'Fast food delivery', 1)`);
    ...
    await db.exec(`INSERT INTO orders (id, public_id, service_id, assigned_driver_id, assigned_vehicle_id, status, created_at) VALUES ('order_1', '${order1PublicId}', 'service_1', 'driver_a1', 'vehicle_v1', 'delivered', ${Math.floor(yesterday.getTime() / 1000)})`);
    await db.exec(`INSERT INTO orders (id, public_id, service_id, assigned_driver_id, assigned_vehicle_id, status, created_at) VALUES ('order_2', '${order2PublicId}', 'service_1', 'driver_a1', 'vehicle_v2', 'delivered', ${Math.floor(today.getTime() / 1000)})`);

    // Create order stops
    await db.exec(`INSERT INTO order_stops (id, order_id, sequence, type, lat, lng, address, status) VALUES ('stop_1_pickup', 'order_1', 1, 'pickup', -6.200000, 106.816666, 'Jl. Merdeka No. 5', 'completed')`);
    await db.exec(`INSERT INTO order_stops (id, order_id, sequence, type, lat, lng, address, status) VALUES ('stop_1_dropoff', 'order_1', 2, 'dropoff', -6.208889, 106.845833, 'Jl. Sudirman No. 10', 'completed')`);
    await db.exec(`INSERT INTO order_stops (id, order_id, sequence, type, lat, lng, address, status) VALUES ('stop_2_pickup', 'order_2', 1, 'pickup', -6.175000, 106.827778, 'Jl. Pahlawan No. 1', 'completed')`);
    await db.exec(`INSERT INTO order_stops (id, order_id, sequence, type, lat, lng, address, status) VALUES ('stop_2_dropoff', 'order_2', 2, 'dropoff', -6.183333, 106.833333, 'Jl. Veteran No. 2', 'completed')`);
