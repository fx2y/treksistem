<script lang="ts">
	...
	onMount(async () => {
		await Promise.all([loadMasterData(), loadService()]);
	});
	...
	async function loadService() {
		try {
			service = await apiClient.get<Service>(`/mitra/services/${serviceId}`);
			if (service) {
				formData = {
					name: service.name,
					isPublic: service.isPublic,
					maxRangeKm: service.maxRangeKm,
					baseFee: service.rate.baseFee,
					feePerKm: service.rate.feePerKm,
					supportedVehicleTypeIds: [...service.supportedVehicleTypeIds],
					supportedPayloadTypeIds: [...service.supportedPayloadTypeIds],
					availableFacilityIds: [...service.availableFacilityIds]
				};
			}
		} catch (err) {
	...
	async function handleSubmit() {
		...
		try {
			...
			await apiClient.put<Service>(`/mitra/services/${serviceId}`, serviceData);
			await goto('/services');
		} catch (err) {
...
<div class="space-y-6">
	...
	{:else if !masterData || !service}
		<div class="text-center py-12">
			<div class="text-red-600 text-sm">{error}</div>
			<button
				type="button"
				on:click={() => Promise.all([loadMasterData(), loadService()])}
